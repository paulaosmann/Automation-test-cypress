...
Cypress.Commands.add("fillOutCreditCardForm", (details) => {
  cy.get(".__PrivateStripeElement > iframe")
    .iframe()
    .then((iframes) => {
      cy.wrap(iframes[0]).find(".InputElement").first().type(details.number);
      cy.wrap(iframes[1]).find(".InputElement").first().type(details.date);
      cy.wrap(iframes[2]).find(".InputElement").first().type(details.cvc);
    });
});
